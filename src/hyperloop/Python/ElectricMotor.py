...
class ElectricMotor(Component):
    ...
    def __init__(self):
        ...
        self.add_param('Pmax',
                       val=0.0,
                       desc='Conversion of DesignPower in hp to Watts',
                       units='W')
        self.add_param('D2L',
                       val=0.0,
                       desc='D-squared*L parameter which is ~ to Torque',
                       units='mm**3')
        self.add_param('D2L_ft',
                       val=0.0,
        ...
        self.add_param('wmax',
                       val=0.0,
                       desc='Max speed in rad/s',
                       units='rad/s')

        # Desired outputs
        self.add_output('Current',
                        val=2.0,
                        desc='AC `input current magnitude',
                        units='Amps')
    ...
    def solve_nonlinear(self, params, unknowns, resids):
        ...
        unknowns['phaseCurrent'] = self.phaseCurrent_calc(Pmax, Max_RPM, wmax,
                                                          nphase, Kt, I0)
        phaseCurrent = unknowns['phaseCurrent']

        unknowns['phaseVoltage'] = self.phaseVoltage_calc(w, phaseCurrent,
                                                          nphase, R0, Kv)
...
if __name__ == '__main__':
    ...
    print('Motor Size (Volume=pi*radius^2*L) [ft^3]: %f' %
          prob['comp.Volume_ft'])
