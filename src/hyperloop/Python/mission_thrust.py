...
class MissionThrust(Component):
    ...
    def solve_nonlinear(self, params, unknowns, resids):
        ...
        unknowns['Thrust'] = (m_pod * g) * (
            1 + sin(theta)) + .5 * Cd * rho * S * (V**2) + D_magnetic
...
if __name__ == '__main__':
    ...
    params = (('V', 335.0, {'units': 'm/s'}),
              ('Thrust_pod', 3500.0, {'units': 'N'}),
              ('D_magnetic', 150.0, {'units': 'N'}),
              ('theta', 0.0, {'units': 'rad'}), ('Cd', .2))

    root.add('input_vars',
             IndepVarComp(params),
             promotes=['V', 'Thrust_pod', 'D_magnetic', 'theta', 'Cd'])
    root.add(
        'p',
        MissionThrust(),
        promotes=['V', 'Thrust_pod', 'D_magnetic', 'theta', 'Cd', 'Thrust'])
